...
from viper import conversation as conversation_lib
from viper.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                             DEFAULT_IMAGE_TOKEN, IGNORE_INDEX,
                             IMAGE_TOKEN_INDEX)
from viper.mm_utils import process_anyres_image, tokenizer_image_token
from viper.model import *
from viper.train.llava_trainer import LLaVATrainer
...
def preprocess_system(
    ...
    targets = input_ids.clone()
    assert conv.sep_style == conversation_lib.SeparatorStyle.SYSTEM
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for conv_idx in range(3, len(rounds), 2):
            re_rounds.append(conv.sep.join(
                rounds[conv_idx:conv_idx+2]) + conv.sep)    # user + gpt
        cur_len = 0
        target[:cur_len] = IGNORE_INDEX
        for i, rou in enumerate(re_rounds):
            if rou == "":
...
def preprocess_system_phi(
    ...
    targets = input_ids.clone()
    assert conv.sep_style == conversation_lib.SeparatorStyle.SYSTEM_PHI
    ...
    for conversation, target in zip(conversations, targets):
        ...
        rounds = conversation.split(conv.sep)
        if rounds[-1] == "<|endoftext|>":
            #rounds[-2] = rounds[-2] + rounds[-1]
            rounds = rounds[:-1]
        re_rounds = [conv.sep.join(rounds[:3]) + conv.sep]  # system + user + gpt
        for conv_idx in range(3, len(rounds), 2):
            re_rounds.append(conv.sep.join(
        ...
        for i, rou in enumerate(re_rounds):
            ...
            # if i != 0 and getattr(tokenizer, 'legacy', False) and IS_TOKENIZER_GREATER_THAN_0_14:
            if i != 0 :
                # conpensate for the end of text token
                round_len -= 1
                instruction_len -= 1
...
def preprocess_system_falcon_mamba(
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    has_image: bool = False
) -> Dict:
    ...
    targets = input_ids.clone()
    assert conv.sep_style == conversation_lib.SeparatorStyle.SYSTEM_FALCON_MAMBA
...
def preprocess_system_jamba(
    ...
    targets = input_ids.clone()
    assert conv.sep_style == conversation_lib.SeparatorStyle.SYSTEM_JAMBA
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "system":
        return preprocess_system(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "system_phi":
        return preprocess_system_phi(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "system_falcon_mamba":
        return preprocess_system_falcon_mamba(sources, tokenizer, has_image=has_image)
    if conversation_lib.default_conversation.version == "system_jamba":
        return preprocess_system_jamba(sources, tokenizer, has_image=has_image)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
def train(attn_implementation=None):
    ...
    if 'llava-v1.6-8b' not in model_args.model_name_or_path:
        ...
        if "falcon" in model_args.model_name_or_path.lower():
            # manually set the max_position_embeddings to a large number
            config.max_position_embeddings = 1000000000000000019884624838656
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            config = transformers.AutoConfig.from_pretrained(
                model_args.model_name_or_path, trust_remote_code=True)
            config.max_position_embeddings = 1000000000000000019884624838656
            model = LlavaFalconMambaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
        ...
        elif "jamba" in model_args.model_name_or_path.lower():
            config = transformers.AutoConfig.from_pretrained(
                model_args.model_name_or_path, trust_remote_code=True)
            model = LlavaJambaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                attn_implementation=attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    elif "falcon_mamba" in model_args.model_name_or_path.lower():
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
    ...
    if model_args.version == "system":
        # add tokenizer to the system conversation
        if model.config.max_position_embeddings < training_args.model_max_length:
            model.config.max_position_embeddings = training_args.model_max_length
        if tokenizer.model_max_length < training_args.model_max_length:
            tokenizer.model_max_length = training_args.model_max_length
        conversation_lib.default_conversation.tokenizer = tokenizer
        conversation_lib.default_conversation.sep = "<|im_end|>\n"

    if model_args.version == "system_phi":
        # add tokenizer to the system conversation
        if model.config.max_position_embeddings < training_args.model_max_length:
            model.config.max_position_embeddings = training_args.model_max_length
        if tokenizer.model_max_length < training_args.model_max_length:
            tokenizer.model_max_length = training_args.model_max_length
        conversation_lib.default_conversation.tokenizer = tokenizer
        conversation_lib.default_conversation.sep = "<|end|>\n"

    if model_args.version == "system_falcon_mamba":
        # add tokenizer to the system conversation
        if model.config.max_position_embeddings < training_args.model_max_length:
            model.config.max_position_embeddings = training_args.model_max_length
        if tokenizer.model_max_length < training_args.model_max_length:
    ...
    if model_args.version == "system_jamba":
        # add tokenizer to the system conversation
        if model.config.max_position_embeddings < training_args.model_max_length:
            model.config.max_position_embeddings = training_args.model_max_length
        if tokenizer.model_max_length < training_args.model_max_length:
            tokenizer.model_max_length = training_args.model_max_length
        conversation_lib.default_conversation.tokenizer = tokenizer
        conversation_lib.default_conversation.sep = "<|eom|>"
    ...
    if model_args.vision_tower is not None:
        ...
        if getattr(vision_tower, "config", None) is None:
            vision_tower.config = lambda: None
            vision_tower.config.image_size = vision_tower._image_size

        model.config.use_image_rope = data_args.use_image_rope
        model.config.image_aspect_ratio = data_args.image_aspect_ratio
        if data_args.image_aspect_ratio == 'anyres':
            base_size = vision_tower.config.image_size
            #grids = [[1, 2], [2, 1], [2, 2], [3, 1], [1, 3]]
            #grids = [[2, 3], [3, 2], [3, 3], [4, 1], [1, 4], [4, 2], [2, 4]]
            # grids = [[5, 5], [6, 4], [4, 6], [7, 3], [3, 7], [8, 2], [2, 8], [9, 1], [1, 9], [8, 3], [3, 8], [9, 2], [2, 9]]
            grids = []
            for x in range(1, 16):
                for y in range(1, 16):
                    if x * y <= 16:
